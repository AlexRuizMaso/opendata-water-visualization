...
\begin{enumerate}
    ...
    \item \textbf{Fase 2 - Pipeline ETL base}: Implementació dels extractors de dades, transformadors de dades i loaders de fitxers.
...
\subsection{Treball en equip i comunicació}

Tot i ser un projecte desenvolupat per un únic autor, s'han mantingut reunions periòdiques amb el tutor acadèmic (Antoni Bardera Reig) per a revisió de progrés, discussió de decisions tècniques i ajust del pla de treball. Concretament:

\begin{itemize}
    \item \textbf{Reunions de seguiment}: S'han realitzat reunions quinzenals amb el tutor (aproximadament 12 hores en total) per a presentar avanços, discutir problemes tècnics i rebre feedback sobre l'arquitectura del sistema.
    \item \textbf{Consultes informals}: S'han mantingut interaccions esporàdiques amb companys del grau per a validació de supòsits d'usabilitat i obtenció de perspectives d'usuari no tècnic (aproximadament 10 hores).
    \item \textbf{Documentació de decisions}: Totes les decisions tècniques significatives es van documentar al repositori Git mitjançant missatges de commit descriptius, permetent la traçabilitat de l'evolució del codi.
\end{itemize}

\subsection{Testos unitaris i validació}

S'han implementat testos unitaris per a cada bloc funcional del sistema, assegurant la qualitat del codi i facilitant la detecció precoç d'errors:

\begin{itemize}
    \item \textbf{Pipeline ETL}: Testos unitaris per a cada mòdul (extractors, transformadors, carregadors) amb llibreria Vitest, verificant estructures de dades, normalització de dates, fusió de registres i neteja de duplicats. Cobertura de codi objectiu: 90\%.
    \item \textbf{Frontend}: Testos de components React per a validar el correcte funcionament dels dashboards, la navegació entre panells, i la integració amb les biblioteques de visualització (Recharts, Leaflet).
    \item \textbf{Integració}: Proves d'integració que validen el flux complet des de l'extracció de dades fins a la seva visualització al frontend, incloent la simulació d'errors de xarxa i condicions adverses.
    \item \textbf{Usabilitat}: Proves d'usabilitat amb 5 usuaris externs (tècnics i no tècnics) per a validar la intuïtitivitat de la interfície i detectar problemes d'experiència d'usuari.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Setmanes 3-5}: Desenvolupament del pipeline ETL (extractors, transformadors, loaders).
...
\begin{itemize}
    ...
    \item Les revisions de progrés i ajusts del pla de treball es van realitzar mitjançant intercanvi de correus electrònics amb el tutor, permetent un seguiment continu del desenvolupament.
\end{itemize}
...
\begin{itemize}
    ...
    \item Proves unitàries de cada mòdul ETL amb Vitest (90\% de cobertura de codi) - 8 hores
...
\begin{itemize}
    ...
    \item Creació de hooks personalitzats per a gestió d'estat (useState, useEffect) - 5 hores
...
\begin{itemize}
    \item Desenvolupament del Dashboard 3 - Correlació clima-aigua: gràfic de dispersió, algoritme d'agregació, controls interactius - 16 hores
...
\begin{itemize}
    ...
    \item Elaboració del vídeo demostratiu mostrant funcionalitats clau - 4 hores
...
\hline
\textbf{Fase} & \textbf{Tasques principals} & \textbf{Est.} & \textbf{Real} \\
\hline
1 & Investigació i configuració inicial & 30 & 31 \\
\hline
2 & Desenvolupament del pipeline ETL & 60 & 62 \\
\hline
3 & Frontend - Dashboards 1 i 2 & 67 & 69 \\
\hline
4 & Frontend - Dashboards 3 i 4 & 49 & 51 \\
\hline
5 & Automatització, desplegament i documentació & 52 & 52 \\
\hline
6 & Testing i validació & 42 & 45 \\
\hline
Tutories i revisions & Reunions amb el tutor & 12 & 12 \\
\hline
Reserva d'imprevistos & Flexibilitat & 18 & 19 \\
\hline
\textbf{TOTAL} & & \textbf{330} & \textbf{341} \\
\hline
\end{tabular}
\label{tab:time-estimation}
\end{table}

L'estimació total de 330 hores es va comparar amb la inversió real documentada de 341 hores, resultant un error de +3,2\%. Les principals desviacions van ocórrer en les fases de desenvolupament frontend (sobrecost de 4 hores per optimitzacions de rendiment) i testing (sobrecost de 3 hores per proves d'usabilitat). En general, l'estimació va resultar prou precisa, indicant una bona comprensió de la complexitat tècnica involucrada.
...
\begin{itemize}
    ...
    \item \textbf{S'explica als}: Capítol 8 (decisions i implementació del pipeline ETL).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{S'explica als}: Capítols 8 (decisions i implementació del frontend), 9 (desplegament i validació).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{S'explica als}: Capítol 9 (implantació, desplegament i validació de requisits no funcionals).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Documentació}: La memòria del TFG i la documentació tècnica s'han redactat amb assistència de IA per a adaptar els textos del desenvolupador al format LaTeX i millorar la claredat expositiva, verificant sempre la precisió tècnica del contingut.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Incidència}: Les dades no es carregaven correctament a tots els dashboards tot i estar emmagatzemades als fitxers JSON.
    \textbf{Resolució}: Revisió del flux de càrrega de dades al frontend, verificant la ruta dels fitxers i la gestió d'errors asincrònics.
    
    \item \textbf{Incidència}: Els punts geogràfics del mapa del Dashboard 1 eren incorrectes.
    \textbf{Resolució}: Correcció de les coordenades de latitud/longitud de cada embassament al transformer.
